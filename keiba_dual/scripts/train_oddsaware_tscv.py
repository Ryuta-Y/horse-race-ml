# keiba_dual_ready_full/scripts/train_oddsaware_tscv.py
# -----------------------------------------------------
# 時系列CV(TimeSeriesSplit)で評価しつつ、全期間で再学習して保存。
# 保存: models/oddsaware_tscv.pkl / 履歴: models/oddsaware_tscv_history.csv, .png
# 実行: (venv) keiba_dual_ready_full $ python -m scripts.train_oddsaware_tscv
...
ODDSAWARE_TSCV_MODEL_PATH = os.path.join(MODEL_DIR, "oddsaware_tscv.pkl")
...
def make_features(df):
    ...
    # “当日オッズあり”想定：win_odds由来の特徴が自然に入る（NUM_COLSに win_odds が含まれている）
    Xcat = []
...
def main():
    ...
    hist_path = os.path.join(MODEL_DIR, "oddsaware_tscv_history.csv")
    hist.to_csv(hist_path, index=False)

    bundle = sgd_fit(X, y)
    joblib.dump(bundle, ODDSAWARE_TSCV_MODEL_PATH)
    ...
    try:
        ...
        fig.tight_layout(); fig.savefig(os.path.join(MODEL_DIR,"oddsaware_tscv_history.png")); plt.close(fig)
    except Exception as e:
        print("plot skipped:", e)

    print(f"saved: {ODDSAWARE_TSCV_MODEL_PATH}")
