# scripts/train_oddsaware_lgbm.py
...
def main():
    ...
    ap.add_argument("--out", default="models/oddsaware_lgbm.pkl")
    ap.add_argument("--folds", type=int, default=5)
    ap.add_argument("--early_stopping", type=int, default=50)
    args = ap.parse_args()

    rec = load_records(args.records_glob)
    X, y, groups, feat_cols = build_features(rec, use_odds=True)
    ...
    for fold, (tr, va) in enumerate(sgkf.split(X, y, groups), 1):
        ...
        gbm = lgb.train(
            params, train_set, valid_sets=[valid_set], num_boost_round=2000,
        ...
        oof[va] = p
        print(f"[lgbm-oddsaware] fold{fold}: logloss={log_loss(yva, p):.4f}, auc={roc_auc_score(yva, p):.4f}")
        models.append(gbm)

    print(f"[lgbm-oddsaware] OOF logloss={log_loss(y, oof):.4f}, auc={roc_auc_score(y, oof):.4f}")
    best_its = [m.best_iteration or 100 for m in models]
    final_iter = int(np.median(best_its))
    final_model = lgb.train(params, lgb.Dataset(X, label=y), num_boost_round=final_iter)
    bundle = {"model": final_model, "scaler": None, "columns": list(X.columns)}
    os.makedirs(os.path.dirname(args.out), exist_ok=True)
    joblib.dump(bundle, args.out)
    print(f"[lgbm-oddsaware] features: {list(X.columns)}")
    print(f"[lgbm-oddsaware] saved: {args.out}")
