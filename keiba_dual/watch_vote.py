...
# - oddsaware.pkl / oddsless.pkl があれば自動併用(BOTH_AGREE_BOOST等は bet_policy 側のロジックに依存)。
# - ログを詳細に出力。残ポイントも表示。
#
# 注意:
...
# run_vote と同じ正規化・API関数群をここにも実装（外部依存を減らすため）
...
def _build_and_bet_for(place: str, race_num: int, date_str: str):
    ...
    # プラン作成（内部で get_odds_func=api_get_odds を使って直近オッズを参照）
    from bet_policy import make_plan_for_day
...
def main():
    ...
    print(f"[watch_vote] TARGET_DATE={date_str}, TARGET_PLACE={TARGET_PLACE}, TARGET_RACE={TARGET_RACE}")

    # 初回に当日データからタイムテーブルを取得し、対象スロットを決定
    try:
        rc = api_get_racecards(date_str)
